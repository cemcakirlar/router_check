...
class ProxyHandler(http.server.BaseHTTPRequestHandler):
    ...
    def serve_file(self, filename, content_type):
        # When using Eel with a separate web/ folder
        filepath = resource_path(os.path.join("web", filename))
        try:
...
if __name__ == "__main__":
    ...
    # 3. Give the server a moment to initialize
    time.sleep(1)
    
    # 4. Start Eel, pointing it to our proxy server URL
    print("  → Launching Eel (Chrome App Mode)...")
    try:
        eel.start(
            "index.html", 
